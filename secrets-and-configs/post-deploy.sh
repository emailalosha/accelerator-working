...
echo "03.                VAULT_TOKEN: ${VAULT_TOKEN}"
...
if [ -z "${3}" ]; then
  echo "-> {3} is EMPTY."
  echo "-- Please set to the VAULT_TOKEN for access the secrets path on the VAULT server. E.g x.MO2n7SHKXe15H4uLwRq6tsCN"
  echo ""
  errorFound="true"
fi
  ...
  case "${scriptAction}" in
    ...
    "add-secrets")
      ...
      for filename in hashicorp-vault/*.json; do
          ...
          addSecretsToVault "${VAULT_ADDR}" "${VAULT_TOKEN}" "${secretsPath}" "${secretsEngineName}" "${path_tmpFile}"
          rm -f "${path_tmpFile}"
      done
      ;;
    "del-secrets")
      ...
      for filename in client-config/*.json; do
        ...
        deleteSecretsFromVault_AllVersions "${VAULT_ADDR}" "${VAULT_TOKEN}" "${secretsPath}" "${secretsEngineName}"
      done
      ;;
    *)
